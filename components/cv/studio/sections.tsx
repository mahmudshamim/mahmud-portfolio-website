...
export function sectionStatus(id: SectionId, profile: CVData, version: RoleVersion): Status {
  ...
  switch (id) {
    ...
    case 'work': {
      ...
      return { text: shown < n ? `${plural(n, 'job')} · ${shown} shown` : plural(n, 'job'), state: 'done' }
    }
    case 'education': {
      const n = profile.education.length
      return n ? { text: plural(n, 'entry', 'entries'), state: 'done' } : { text: 'Add a degree or course', state: 'optional' }
    }
    case 'skills': {
      ...
      return { text: `${shown} of ${n} shown${role ? ` for ${role}` : ''}`, state: 'done' }
    }
    case 'projects': {
...
function SwitchHint({ role }: { role: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: radius.md, background: c.brandSoft, fontFamily: font, fontSize: 13, lineHeight: 1.45, color: c.brandInk }}>
      ...
      <span>
        The switch shows or hides an item in your <b>{role || 'current'}</b> CV only. Your other versions keep their own choice.
      </span>
    </div>
  )
}
  ...
  return (
    <div style={{ display: 'grid', gap: 12 }}>
      ...
      {offRole ? (
        ...
        <Note>
          Only the <b>{role || 'current'}</b> version uses this text. Name the tools you want to be hired for — screening software reads this first.
        </Note>
      )}
    </div>
  )
      ...
      {profile.experience.map((job, i) => {
        ...
        return (
          <ItemCard
            ...
            showLabel={`Show ${job.role || 'this job'} in the ${role || 'current'} CV`}
      ...
      {profile.projects.map((p, i) => {
        ...
        return (
          <ItemCard
            ...
            showLabel={`Show ${p.name || 'this project'} in the ${role || 'current'} CV`}
            hiddenNote={`Hidden in the ${role || 'current'} CV`}
          >
