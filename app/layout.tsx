...
import { Exo_2, Montserrat, Poppins, Satisfy } from 'next/font/google'
...
/* Wordmark only — one weight, one style, so it stays a small download. */
const exo2 = Exo_2({
  weight: ['600'],
  style: ['italic'],
  subsets: ['latin'],
...
export const metadata: Metadata = {
  ...
  icons: {
    // SVG first — browsers that understand it get the crisp vector; the rest
    // fall through to the PNG generated by app/icon.tsx.
    // Note the extensionless URLs: app/icon.tsx and app/apple-icon.tsx serve
    // at /icon and /apple-icon, not /icon.png. Declaring `icons` here also
    // suppresses Next's own generated <link> tags, so these must be right.
    icon: [
...
export default function RootLayout({
  ...
  return (
    <html lang="en" className={`${montserrat.variable} ${poppins.variable} ${satisfy.variable} ${exo2.variable}`}>
      <head>
        <script
